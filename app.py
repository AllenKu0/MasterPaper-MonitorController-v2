...
async def alert(request: Request):
    ...
    print("start")
    await cb_controller.circuit(alert_data, cluster)
    print("end")
    # asyncio.create_task(cb_controller.circuit(alert_data, cluster, client_host))
...
async def mirror(request: MirrorRequest):
    ...
    ifname = k.get_pod_info_by_lable(CLUSTER1, "app=ueransim-gnb")['veth_name'] 
    await mirror_controller.load_ebpf_program(mirror_ip_str, mirror_mac, ifname, "cilium_host")
    mirror_repository.mongodb_insert(CLUSTER1, mirror_pod_name, ifname)
...
async def _trace_latency(request: TraceLatencyRequest):    
    ...
    latency_task, idx = await trace_controller.load_ebpf_program(ifname, pod_name)
