...
db = client["monitor"]
collection = db["loadbalancer"]

# insert
def mongodb_insert(name, upf_pod_names):
    doc = {
            "name": name,
            "upf_pod_names": upf_pod_names
        }
    print(f"Inserting for {name}: {doc}")
    try:
...
# updaet
def mongodb_update(name, upf_pod_names):
    # 構建更新條件 (匹配的條件)
    query = {
        "name": name,
        "upf_pod_names": upf_pod_names
    ...
    print(f"Updating for {name}: {query} with {update}")
